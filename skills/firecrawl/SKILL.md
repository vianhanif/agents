...
Before doing real work, verify the gateway is live by calling `firecrawl__firecrawl_search` with a simple query (e.g. "test").

The MCP tools via 9router-gateway are the primary interface for all Firecrawl operations. Fall back to the CLI only when MCP is unavailable.
...
**Prefer MCP tools via 9router-gateway** for all Firecrawl operations (search, scrape, map, crawl, extract, agent, interact, parse, monitor). Use the CLI only as fallback if the MCP gateway is unavailable.

All 9router-gateway Firecrawl tools use the `firecrawl__` prefix:
- `firecrawl__firecrawl_search` — search + optional full-page scrape
- `firecrawl__firecrawl_scrape` — single URL with JS rendering support
- `firecrawl__firecrawl_map` — discover URLs on a site, with optional search
- `firecrawl__firecrawl_crawl` — bulk extraction across pages
- `firecrawl__firecrawl_extract` — structured data extraction via schema
- `firecrawl__firecrawl_agent` — AI-powered autonomous research
- `firecrawl__firecrawl_interact` — click, fill, navigate JS-rendered pages
- `firecrawl__firecrawl_parse` — parse local files (PDF, DOCX, XLSX, HTML)
- `firecrawl__firecrawl_monitor_*` — recurring change tracking
- `firecrawl__firecrawl_research_*` — arXiv papers, GitHub issues
